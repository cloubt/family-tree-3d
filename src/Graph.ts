...
class Graph {
  ...
  static addEdge(parameters: EdgeParameters): void {
    // create
    if (!Graph.vertices.has(parameters.from))
      throw new Error(`Could not find source vertex ${parameters.from}`);
    if (!Graph.vertices.has(parameters.to))
      throw new Error(`Could not find source vertex ${parameters.to}`);
  ...
  static getEdge(from: string, to?: string): Edge {
    if (typeof to === "undefined") {
      ...
      if (typeof edge === "undefined")
        throw new Error(`Edge of uuid ${from} not found!`);
      return edge;
    }
    ...
    if (dest === undefined)
      throw new Error(`target vertex ${to} does not exist!`);
    const _ =
      [...Graph.edges.values()].find(
        (edge) => edge.source === source && edge.target === dest
      ) ??
      [...Graph.edges.values()].find(
        (edge) => edge.source === dest && edge.target === source
      );
    if (_ === undefined)
      throw new Error(`Could not find the edge between ${from} and ${to}!`);
    return _;
  }
  static getNeighborhood(object: string): Array<Edge | Vertex> {
    if (!Graph.vertices.has(object))
      throw new Error(
        `Could not get neighborhood for nonexistent vertex ${object}!`
      );
  ...
  static selectNeighborhood(object: string): boolean {
    if (!Graph.vertices.has(object))
      throw new Error(
        `Invalid vertex name ${object}! Please only provide vertex names`
      );
    // clear set
  ...
  static importData(elements: ImportDataParameters[]): any {
    ...
    elements.forEach((element: ImportDataParameters) => {
      if (element.data === "vertex") {
        ...
        if (
          typeof element.to === "undefined" ||
          typeof element.from === "undefined" ||
          typeof element.directed === "undefined"
        )
          throw new Error(
            `Edge ${element.name} had malformed source and/or target vertices!`
          );
        Graph.addEdge({
          name: element.name.toLowerCase(),
...
class Edge {
  ...
  constructor(parameters: EdgeParameters) {
    ...
    this.lineGeometry = new TubeGeometry(
      this.path,
      this.tube.tubularSegments,
      this.tube.radius,
      this.tube.radialSegments,
      false
    );
...
class DirectedEdge extends Edge {
  ...
  constructor(parameters: EdgeParameters) {
    ...
    // console.debug(this.path.v2)
    const binormal = this.path
      .computeFrenetFrames(3)
      .binormals[1].setLength(this.separation);
    // console.debug(`binormal vector for edge going from ${this.source.name} to ${this.target.name}`)
    // console.debug(binormal)

    this.arrowGeometry = new ConeGeometry(
      this.arrowRadius,
      this.arrowLength,
      7
    );
    this.arrowGeometry.translate(
      0,
      -this.arrowLength / 2 - this.target.radius,
      0
    );
    ...
    console.debug(this.path.isLineCurve3);
    this.path2 = new QuadraticBezierCurve3(
      this.source.position.clone(),
      // binormal vector converted to world coordinates
      this.source.position
        .clone()
        .lerp(this.target.position, 0.5)
        .add(binormal),
